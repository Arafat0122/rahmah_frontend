...
import {
    CheckCircle2, ShieldCheck, BookOpen, Scale, Library, Landmark
} from 'lucide-react';
...
import HanafiFiqhFAQ from "./HanafiFiqhFAQ";

const HanafiFiqhPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (isMounted) {
                    // 1. Services Logic: Filter by "Islamic studies" category + "hanafi" or "fiqh" keywords
                    // Rank by userId (Natural Sort)
                    const filteredServices = serviceRes.data
                        .filter(s =>
                            s.status === "Accepted" &&
                            s.category === "Hanafi Fiqh" ||
                            s.title?.toLowerCase().includes("hanafi")
                        )
                    ...
                    // 2. Teacher Logic: Filter by "Islamic Studies teacher" within category array
                    // Rank by userId
                    const filteredTeachers = teacherRes.data
                        .filter(t =>
                            t.role === "teacher" &&
                            Array.isArray(t.category) &&
                            t.category.includes("Fiqh Hanafi Teacher") ||
                            t.category.includes("Usule Fiqh Teacher")
                        )
                        .sort((a, b) => (a.userId || "").localeCompare(b.userId || "", undefined, { numeric: true }))
                        .slice(0, 4);
                ...
                console.error("Error fetching Hanafi Fiqh data:", err);
            } finally {
    ...
    const schemaData = {
        ...
        "@type": "Course",
        "name": "Hanafi Fiqh Mastery Course",
        "description": "A comprehensive study of Islamic Jurisprudence according to the Hanafi Madhhab, covering Ibadat, Muamalat, and Munakahat.",
        "provider": {
            "@type": "Organization",
        ...
        <main className="min-h-screen bg-[#FBFDFF] pb-20 font-body">
            ...
            <section className="relative bg-white pt-24 pb-16 px-6 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative z-10 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-slate-800 mb-8">
                        <Landmark size={14} className="text-primary fill-primary" />
                        <span className="text-[10px] font-black uppercase tracking-widest text-white">Legacy of Imam Abu Hanifa</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-serif font-bold text-slate-900 mb-6 tracking-tight leading-[1.1]">
                        Ground Your Faith in <br />
                        <span className="text-primary italic font-medium">Hanafi Jurisprudence</span>
                    </h1>
                    <p className="max-w-2xl mx-auto text-base md:text-lg text-slate-600 mb-10 leading-relaxed">
                        Master the rules of worship and daily life according to the <span className="text-slate-900 font-semibold">Hanafi Madhhab</span>. A systematic study from foundational primers to advanced legal methodology.
                    </p>
            ...
            <section className="max-w-5xl mx-auto px-6 -mt-8 relative z-20">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white p-2 rounded-[2.5rem] shadow-xl border border-slate-100">
                    {[
                        { icon: <Scale size={18} />, label: "Ibadat", sub: "Worship" },
                        { icon: <Library size={18} />, label: "Usul", sub: "Principles" },
                        { icon: <ShieldCheck size={18} />, label: "Muamalat", sub: "Transactions" },
                        { icon: <BookOpen size={18} />, label: "Authentic", sub: "Tradition" },
                    ].map((stat, i) => (
            ...
            {/* --- 3. FIQH SERVICES (Ranked) --- */}
            <section className="max-w-7xl mx-auto px-6 py-24">
                <div className="mb-12">
                    <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-900">Course Modules</h2>
                    <p className="text-slate-500 mt-2">Structured learning paths from basic ritual purity to complex family law.</p>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {fiqhServices.length > 0 ? (
                            ...
                            <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-100 rounded-[3rem]">
                                <p className="text-slate-400 font-serif">Hanafi Fiqh modules are being updated.</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            <section className="max-w-7xl mx-auto px-6 py-20 bg-slate-50 rounded-[4rem]">
                <header className="mb-12 text-center">
                    <h2 className="text-3xl font-serif font-bold text-slate-900">Qualified Muftis & Instructors</h2>
                    <p className="text-slate-500 font-medium mt-2">Specialists in the Hanafi legal tradition with Ijazah in classical texts.</p>
                </header>
                {loading ? (
                ...
                <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center gap-16">
                    <div className="flex-1 space-y-8">
                        <h2 className="text-3xl md:text-5xl font-serif font-bold text-white leading-tight">
                            The Analytical Power <br />
                            <span className="text-primary">of Hanafi Fiqh</span>
                        </h2>
                        <div className="space-y-6">
                            {[
                                { title: "Usul al-Fiqh", desc: "Understanding the foundational principles and legal reasoning used to derive rulings." },
                                { title: "Applied Ibadat", desc: "Detailed study of Salah, Zakat, Sawm, and Hajj according to the school's dominant views." },
                                { title: "Contemporary Issues", desc: "Applying the Hanafi framework to modern financial and ethical dilemmas." }
                            ].map((step, i) => (
                    ...
                    <div className="flex-1 w-full lg:max-w-md bg-white/5 rounded-[3rem] p-8 border border-white/10 backdrop-blur-sm">
                        <h3 className="text-xl font-serif font-bold text-white mb-6">Core Curriculum Texts</h3>
                        <div className="space-y-4">
                            {["Nur al-Idah (Light of Clarification)", "Mukhtasar al-Quduri", "Al-Hidayah (Introductory Analysis)", "Fatawa Hindiyya Overview", "Usul al-Shashi"].map((module, i) => (
