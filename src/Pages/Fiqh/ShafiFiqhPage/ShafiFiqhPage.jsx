...
import {
    CheckCircle2, ShieldCheck, BookOpen, Anchor, Compass, PenTool
} from 'lucide-react';
...
import ShafiFiqhFAQ from "./ShafiFiqhFAQ";
...
const ShafiFiqhPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (isMounted) {
                    // 1. Services Logic: Filter by "Shafee Fiqh" (Mapped to your constants)
                    const filteredServices = serviceRes.data
                        .filter(s =>
                            s.status === "Accepted" &&
                            s.category === "Shafee Fiqh"
                        )
                    ...
                    // 2. Teacher Logic: Filter by "Fiqh Shafey Teacher"
                    const filteredTeachers = teacherRes.data
                        .filter(t =>
                            t.role === "teacher" &&
                            Array.isArray(t.category) &&
                            t.category.includes("Fiqh Shafey Teacher")
                        )
                        .sort((a, b) => (a.userId || "").localeCompare(b.userId || "", undefined, { numeric: true }))
                        .slice(0, 4);
                ...
                console.error("Error fetching Shafi Fiqh data:", err);
            } finally {
    ...
    const schemaData = {
        ...
        "@type": "Course",
        "name": "Shafi Fiqh Mastery Program",
        "description": "Systematic study of Islamic law according to the Shafi Madhhab, focusing on the evidence-based approach of Imam al-Shafi.",
        "provider": {
            "@type": "Organization",
        ...
        <main className="min-h-screen bg-[#FBFDFF] pb-20 font-body">
            ...
            <section className="relative bg-white pt-24 pb-16 px-6 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative z-10 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-slate-800 mb-8">
                        <Compass size={14} className="text-primary fill-primary" />
                        <span className="text-[10px] font-black uppercase tracking-widest text-white">The Legacy of Imam al-Shafi</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-serif font-bold text-slate-900 mb-6 tracking-tight leading-[1.1]">
                        Master the Evidence-Based <br />
                        <span className="text-primary italic font-medium">Shafi Madhhab</span>
                    </h1>
                    <p className="max-w-2xl mx-auto text-base md:text-lg text-slate-600 mb-10 leading-relaxed">
                        Journey through the legal school of the <span className="text-slate-900 font-semibold">Architect of Usul</span>. Learn to navigate the Shafi methodology from foundational primers to the heights of comparative law.
                    </p>
            ...
            <section className="max-w-5xl mx-auto px-6 -mt-8 relative z-20">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white p-2 rounded-[2.5rem] shadow-xl border border-slate-100">
                    {[
                        { icon: <Anchor size={18} />, label: "Methodology", sub: "Clear Usul" },
                        { icon: <PenTool size={18} />, label: "Authentic", sub: "Classic Texts" },
                        { icon: <ShieldCheck size={18} />, label: "Dalil", sub: "Evidence" },
                        { icon: <BookOpen size={18} />, label: "Sanad", sub: "Linked Chain" },
                    ].map((stat, i) => (
            ...
            <section className="max-w-7xl mx-auto px-6 py-24">
                <div className="mb-12">
                    <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-900">Learning Tracks</h2>
                    <p className="text-slate-500 mt-2">Comprehensive modules covering all aspects of Shafi jurisprudence.</p>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {fiqhServices.length > 0 ? (
                            ...
                            <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-100 rounded-[3rem]">
                                <p className="text-slate-400 font-serif">Shafee Fiqh modules are being updated.</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            <section className="max-w-7xl mx-auto px-6 py-20 bg-slate-50 rounded-[4rem]">
                <header className="mb-12 text-center">
                    <h2 className="text-3xl font-serif font-bold text-slate-900">Expert Shafi Instructors</h2>
                    <p className="text-slate-500 font-medium mt-2">Scholars specializing in the legal thought of Imam Muhammad ibn Idris al-Shafi.</p>
                </header>
                {loading ? (
                ...
                <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center gap-16">
                    <div className="flex-1 space-y-8">
                        <h2 className="text-3xl md:text-5xl font-serif font-bold text-white leading-tight">
                            The Analytical Core <br />
                            <span className="text-primary">of Shafi Thought</span>
                        </h2>
                        <div className="space-y-6">
                            {[
                                { title: "Foundational Primers", desc: "Mastering the essentials of worship (Ibadat) using texts like Safinat al-Naja and Matn Abi Shuja." },
                                { title: "Usul al-Shafi", desc: "Understanding the synthesis between Hadith and legal logic established in the Risala." },
                                { title: "Legal Evolution", desc: "Exploring the Qawl Qadim (Old School) and Qawl Jadid (New School) through Minhaaj al-Talibin." }
                            ].map((step, i) => (
                    ...
                    <div className="flex-1 w-full lg:max-w-md bg-white/5 rounded-[3rem] p-8 border border-white/10 backdrop-blur-sm">
                        ...
                        <div className="space-y-4">
                            {["Safinat al-Naja Mastery", "Matn al-Ghayah wa al-Taqrib", "Al-Yaqut al-Nafis Study", "Introduction to Ar-Risala", "Comparative Shafi Rulings"].map((module, i) => (
