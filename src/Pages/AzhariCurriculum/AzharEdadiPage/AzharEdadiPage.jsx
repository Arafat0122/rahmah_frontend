...
import {
    CheckCircle2, ShieldCheck, BookOpen, Compass, Library, Binary
} from 'lucide-react';
...
import AzharEdadiFAQ from "./AzharEdadiFAQ";

const AzharEdadiPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (isMounted) {
                    // 1. Services Logic: Filter by "Azhari curriculum" + "edadi" keyword
                    const filteredServices = serviceRes.data
                        .filter(s =>
                            s.status === "Accepted" &&
                            s.category === "Azhari curriculum" ||
                            s.title?.toLowerCase().includes("edadi")
                        )
                ...
                console.error("Error fetching Azhar Edadi data:", err);
            } finally {
    ...
    const schemaData = {
        ...
        "@type": "Course",
        "name": "Al-Azhar Edadi Islamic Curriculum",
        "description": "The preparatory stage of the Al-Azhar educational system, introducing logic, advanced grammar, and specialized jurisprudence.",
        "provider": {
            "@type": "Organization",
        ...
        <main className="min-h-screen bg-[#FBFDFF] pb-20 font-body">
            ...
            <section className="relative bg-white pt-24 pb-16 px-6 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative z-10 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-slate-800 mb-8">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-white">The Preparatory Path</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-serif font-bold text-slate-900 mb-6 tracking-tight leading-[1.1]">
                        Academic Depth: <br />
                        <span className="text-primary italic font-medium">Al-Azhar Edadi Program</span>
                    </h1>
                    <p className="max-w-2xl mx-auto text-base md:text-lg text-slate-600 mb-10 leading-relaxed">
                        Transitioning from basics to academic excellence. The <span className="text-slate-900 font-semibold">Edadi Stage</span> introduces students to the analytical tools of the Islamic sciences, including Mantiq (Logic) and Usul.
                    </p>
            ...
            <section className="max-w-5xl mx-auto px-6 -mt-8 relative z-20">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white p-2 rounded-[2.5rem] shadow-xl border border-slate-100">
                    {[
                        { icon: <Library size={18} />, label: "Academic", sub: "Rigor" },
                        { icon: <Binary size={18} />, label: "Logic", sub: "Mantiq" },
                        { icon: <ShieldCheck size={18} />, label: "Advanced", sub: "Usul" },
                        { icon: <BookOpen size={18} />, label: "Heritage", sub: "Classical Texts" },
                    ].map((stat, i) => (
            ...
            <section className="max-w-7xl mx-auto px-6 py-24">
                <div className="mb-12">
                    <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-900">Preparatory Modules</h2>
                    <p className="text-slate-500 mt-2">Specialized subjects forming the core of the Edadi academic cycle.</p>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {azharServices.length > 0 ? (
                            ...
                            <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-100 rounded-[3rem]">
                                <p className="text-slate-400 font-serif">Azhari Edadi modules are currently being prepared.</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            <section className="max-w-7xl mx-auto px-6 py-20 bg-slate-50 rounded-[4rem]">
                <header className="mb-12 text-center">
                    <h2 className="text-3xl font-serif font-bold text-slate-900">Academic Mentors</h2>
                    <p className="text-slate-500 font-medium mt-2">Specialists in preparatory Islamic pedagogy and advanced Arabic linguistics.</p>
                </header>
                {loading ? (
                ...
                <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center gap-16">
                    <div className="flex-1 space-y-8">
                        <h2 className="text-3xl md:text-5xl font-serif font-bold text-white leading-tight">
                            Elevating the <br />
                            <span className="text-primary">Azhari Mindset</span>
                        </h2>
                        <div className="space-y-6">
                            {[
                                { title: "Analytical Grammar", desc: "Moving from basic rules to understanding syntax and morphology (Nahw & Sarf) in complex texts." },
                                { title: "Introduction to Mantiq", desc: "Developing sound reasoning and logical proof structures as used in Islamic theology." },
                                { title: "Comparative Fiqh Basics", desc: "Understanding the proofs behind rulings and the diversity of the four schools." }
                            ].map((step, i) => (
                    ...
                    <div className="flex-1 w-full lg:max-w-md bg-white/5 rounded-[3rem] p-8 border border-white/10 backdrop-blur-sm">
                        <h3 className="text-xl font-serif font-bold text-white mb-6">Preparatory Syllabus</h3>
                        <div className="space-y-4">
                            {["Intermediate Tajwid & Hifz", "Applied Arabic Grammar", "Introduction to Usul al-Fiqh", "Biographies of Great Scholars", "Tafsir of Key Juzz"].map((module, i) => (
