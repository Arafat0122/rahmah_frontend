...
import {
    CheckCircle2, ShieldCheck, Scroll, Award, History, Zap
} from 'lucide-react';
...
import AzharSanabiFAQ from "./AzharSanabiFAQ";

const AzharSanabiPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (isMounted) {
                    // 1. Services Logic: Filter by "Azhari curriculum" + "sanabi" or "thanawiya"
                    const filteredServices = serviceRes.data
                        .filter(s =>
                            s.status === "Accepted" &&
                            s.category === "Azhari curriculum" ||
                            (s.title?.toLowerCase().includes("sanabi") || s.title?.toLowerCase().includes("thanawiya"))
                        )
                ...
                console.error("Error fetching Azhar Sanabi data:", err);
            } finally {
    ...
    const schemaData = {
        ...
        "@type": "Course",
        "name": "Al-Azhar Sanabi Advanced Curriculum",
        "description": "The secondary stage of Al-Azhar education, preparing students for university-level Islamic scholarship and leadership.",
        "provider": {
            "@type": "Organization",
        ...
        <main className="min-h-screen bg-[#FBFDFF] pb-20 font-body">
            ...
            <section className="relative bg-white pt-24 pb-16 px-6 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative z-10 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-slate-800 mb-8">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-white">Advanced Secondary Excellence</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-serif font-bold text-slate-900 mb-6 tracking-tight leading-[1.1]">
                        ...
                        <span className="text-primary italic font-medium">Al-Azhar Sanabi Stage</span>
                    </h1>
                    <p className="max-w-2xl mx-auto text-base md:text-lg text-slate-600 mb-10 leading-relaxed">
                        The gateway to specialized scholarship. The <span className="text-slate-900 font-semibold">Sanabi Program</span> hones the student&rsquo;s ability to engage with classical primary sources and modern contemporary legal challenges.
                    </p>
            ...
            <section className="max-w-5xl mx-auto px-6 -mt-8 relative z-20">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white p-2 rounded-[2.5rem] shadow-xl border border-slate-100">
                    {[
                        { icon: <Scroll size={18} />, label: "Advanced", sub: "Rhetoric" },
                        { icon: <Zap size={18} />, label: "Critical", sub: "Analysis" },
                        { icon: <ShieldCheck size={18} />, label: "Specialized", sub: "Fiqh" },
                        { icon: <History size={18} />, label: "History", sub: "Tashri'" },
                    ].map((stat, i) => (
            ...
            <section className="max-w-7xl mx-auto px-6 py-24">
                <div className="mb-12">
                    <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-900">Advanced Modules</h2>
                    <p className="text-slate-500 mt-2">Comprehensive subject matter for the final Al-Azhar secondary years.</p>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {azharServices.length > 0 ? (
                            ...
                            <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-100 rounded-[3rem]">
                                <p className="text-slate-400 font-serif">Azhari Sanabi modules are being reviewed by our board.</p>
                            </div>
                        )}
                    </div>
                )}
            ...
            <section className="max-w-7xl mx-auto px-6 py-20 bg-slate-50 rounded-[4rem]">
                <header className="mb-12 text-center">
                    <h2 className="text-3xl font-serif font-bold text-slate-900">Distinguished Scholars</h2>
                    <p className="text-slate-500 font-medium mt-2">Professors and Shuyookh specializing in the highest levels of the Azhari system.</p>
                </header>
                {loading ? (
                ...
                <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center gap-16">
                    <div className="flex-1 space-y-8">
                        <h2 className="text-3xl md:text-5xl font-serif font-bold text-white leading-tight">
                            The Pinnacle of <br />
                            <span className="text-primary">Traditional Mastery</span>
                        </h2>
                        <div className="space-y-6">
                            {[
                                { title: "Historical Jurisprudence", desc: "Studying the history of Islamic Law (Tashri') to understand how rulings have adapted over time." },
                                { title: "Balagha & Eloquence", desc: "Advanced Arabic rhetoric (Bayan, Ma'ani, Badi') used to uncover the linguistic miracles of the Quran." },
                                { title: "Critical Hadith Science", desc: "Moving beyond basic terminology to the evaluation of chains and the science of Rijal." }
                            ].map((step, i) => (
                    ...
                    <div className="flex-1 w-full lg:max-w-md bg-white/5 rounded-[3rem] p-8 border border-white/10 backdrop-blur-sm">
                        <h3 className="text-xl font-serif font-bold text-white mb-6">Secondary Syllabus</h3>
                        <div className="space-y-4">
                            {["Advanced Usul al-Fiqh", "Mustalah al-Hadith (Al-Bayquniyah & Beyond)", "Islamic Philosophy (Mantiq & Kalam)", "History of the Caliphate", "Advanced Tafsir Analysis"].map((module, i) => (
...
export default AzharSanabiPage;